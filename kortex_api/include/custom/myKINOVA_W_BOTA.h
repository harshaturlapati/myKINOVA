...
public:
	...
	myBOTA set_BOTA(myROB_PARAMS ROB_PARAMS) {
		myBOTA BOTA(ROB_PARAMS.DURATION);
		return BOTA;
	}
	...
	bool ROBOT_Gq(bool gripper_in) {
		...
		try
		{
			BOTA = set_BOTA(ROB_PARAMS); // added on 2025-06-17
			...
			while (!(GetKeyState('Q') & 0x8000))
			{
				...
				if (ROB_LOG.now - ROB_LOG.last >= 1000)
				{
					...
					BOTA.BOTA_READ();
					std::cout << BOTA.acceleration[0] << std::endl;
